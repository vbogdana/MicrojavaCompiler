...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Sun Feb 07 05:21:16 CET 2016
  */
...
class CUP$MJParser$actions {
				...
				if (currentCall.getLevel() > pos) {
					if (currentCall.getName().equals("len")) {
						if (expr.getKind() != Struct.Array)
							...
							RESULT = expr;
					} else if (currentCall.getName().equals("ord")) {
						if (expr != Tab.charType)
							parser.report_error("Semantics error at row " + exprleft + " - incompatible types of " + (pos+1) + ". parameter", null);
						else
							RESULT = expr;
					} else if (currentCall.getName().equals("chr")) {
						if (expr != Tab.intType)
							parser.report_error("Semantics error at row " + exprleft + " - incompatible types of " + (pos+1) + ". parameter", null);
						else
							RESULT = expr;
 ...
 parser.type = type; 
		...
		Obj p = (Obj)((java_cup.runtime.Symbol) CUP$MJParser$stack.elementAt(CUP$MJParser$top-2)).value;
			...
			parser.declareFunction("len");
			parser.declareFunction("ord");
			parser.declareFunction("chr");
