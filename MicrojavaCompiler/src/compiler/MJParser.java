...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Tue Feb 09 04:38:47 CET 2016
  */
public class MJParser extends java_cup.runtime.lr_parser {
  ...
  public java_cup.runtime.Symbol scan()
    ...
    public String objToString(Obj o) {
    	SymbolTableVisitor stv = new Visitor();
    	Visitor.stringType = StringHelper.stringType;
    	Visitor.boolType = StringHelper.boolType;
    ...
    public void dump() {
     ...
     public boolean assignableTo(Obj dst, Struct src) {
    	...
    	if (dstType.getKind() == Struct.Array && dstType.getElemType().equals(Tab.charType) && dstType != StringHelper.stringType&& src == StringHelper.stringType)
			return false;
	...
	public boolean inherits(Struct typeL, Struct typeR) {
 		// TODO Auto-generated method stub
    	 if (inheritanceTable.containsKey(typeR))
 			if (inheritanceTable.get(typeR).contains(typeL))
 				return true;
    	 
    	 if (inheritanceTable.containsKey(typeL))
  			if (inheritanceTable.get(typeL).contains(typeR))
  				return true;
    ...
    throws java.lang.Exception
    {
      ...
      switch (CUP$MJParser$act_num)
		...
		Struct typeR = (Struct)((java_cup.runtime.Symbol) CUP$MJParser$stack.peek()).value;
			...
			if (typeL.getKind() == Struct.Array || typeR.getKind() == Struct.Array  || typeL.getKind() == Struct.Class || typeR.getKind() == Struct.Class) {
				if (op != Code.ne && op != Code.eq)
					...
					if ((typeL == StringHelper.stringType && typeR != StringHelper.stringType) || (typeL != StringHelper.stringType && typeR == StringHelper.stringType))
						parser.report_error("Semantics error at row " + typeLleft + " - incompatible types", null);
					else if (typeL == StringHelper.stringType && typeR == StringHelper.stringType) {						
						//		| adr1 adr2
						Code.put(Code.dup2);
						Code.put(Code.pop);
						Code.put(Code.arraylength);
						...
						//		| rezultat
						RESULT = StringHelper.boolType;
					} else if (((typeL.getKind() == Struct.Array || typeR.getKind() == Struct.Array) && typeL.compatibleWith(typeR))
							|| ((typeL.getKind() == Struct.Class || typeR.getKind() == Struct.Class) && (typeL.compatibleWith(typeR) || parser.inherits(typeL, typeR)))) {
						// poredjenje referenci klasa ili nizova
				...
				if (desig != null) {
			 		if (desig.getKind() != Obj.Meth)
			 			parser.report_error("Semantics error at row " + desigleft + " - variable must be a global function or a class method", null);
							...
							if ((desig.getLevel() > 0 && desig.getFpPos() != -1) || (desig.getLevel() > 1 && desig.getFpPos() == -1)){
		...
		Integer op = (Integer)((java_cup.runtime.Symbol) CUP$MJParser$stack.elementAt(CUP$MJParser$top-1)).value;
				...
				if (desig != null) {
					if (desig.getKind() != Obj.Var && desig.getKind() != Obj.Fld && desig.getKind() != Obj.Elem) {
						parser.report_error("Semantics error at row " + desigleft + " - expression must be an lvalue", null);
					} else {
				  ...
				  if (o == null) {
		 				...
		 				parser.report_info("Function '" + methName + "' definition at row " + methNameleft, null);
		 		  		...
		 		  		if ((superClass != null) && (superClass.getMembers().searchKey(methName) != null))
		 		  			...
		 		  			parser.report_error("Semantics error at row " + methNameleft + " - multiple definition of a class method '" + methName + "'", null);
		 		  }	 
			   	  Tab.openScope();
